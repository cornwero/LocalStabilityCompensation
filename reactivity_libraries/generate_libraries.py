...
usage= "python3.7 generate_libraries.py <starting filename or componenets list: S1,B1,S2,...> outbase>"
...
exports .csv and .txt files to H,B,and I directories, populates 'designed' and 'folded' directories with .db files within each.
'''
def construct(struct,variants,randstem,outbase,item,minlen):
    ...
    i = 1
    all_str = []
    csv = open(directory+item[0]+'/'+outbase+'_'+item+'.csv','w')

    with open(directory+item[0]+'/'+outbase+'_'+item+'.txt','w') as f:
        k = list(struct.keys())
            ...
            if hd(outstr,all_str) and 'AAAAA' not in outstr and check_bad_match(struct,k):
                if '(' not in folded[-len(seq3p):] and ')' not in folded[-len(seq3p):]:
                    if folded[:len(seq5p)] == seq5pdb:
                        ...
                        f.write('>'+ID+'\n')
                        f.write(sequence+'\n')
                        db = open(directory+item[0]+'/designed/'+ID+'.db','w')
                        db.write('>'+ID+'\n')
                        db.write(sequence+'\n')
                        db.write(DB+' (-31.5)\n')
                        db.close()
                        fdb = open(directory+item[0]+'/folded/'+ID+'.db','w')
                        fdb.write('>'+ID+'\n')
                        fdb.write(sequence+'\n')
                        fdb.write(folded)
                        fdb.close()

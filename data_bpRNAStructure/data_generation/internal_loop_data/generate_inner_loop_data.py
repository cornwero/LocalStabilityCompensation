...
def get_data_from_internal_loop(internal_loop, structure):
    """
    Function: get_data_from_internal_loop(internal_loop, structure)
    Description: Function to extract data from a given bulge object
    Parameters:
        (bulge) -- Bulge object -- bulge of interest
        (structure) -- Structure object -- Structure object that the bulge
                            is a part of
    Return Value:
        tuple(bulgeLabel, bulgeLen, bulgeEnergy, energy5p, energy3p) OR None
    """
    if internal_loop.canonical():  # check for valid bulge parameter
        internal_loop_label = internal_loop.label()  # store bulge label
        internal_loop_energy = round(
            internal_loop.energy(), 3)  # store bulge energy

        neighbor5p, neighbor3p = structure.neighbors(
            internal_loop_label, object=True)  # get the 5' and 3' neighbor of the bulge
        # check for valid energy parameters in both neighbors
        if (internal_loop._same5pNeighbors()) and (internal_loop._same3pNeighbors()) and (neighbor5p[0].canonical()) and (neighbor3p[0].canonical()):
            # round energy values to 3 decimal places
            energy5p = round(neighbor5p[0].energy(), 3)
            label5p = neighbor5p[0].label()
            # round energy values to 3 decimal places
            energy3p = round(neighbor3p[0].energy(), 3)
            label3p = neighbor3p[0].label()

            # only return data if all values can be accuratley calculated
            return (internal_loop_label, internal_loop_energy, label5p, energy5p, label3p, energy3p)
...
def get_data_from_structure(structure, rna_identifiers):
    ...
    Parameters:
        (structure) -- Structure object -- Structure object that
                            is being examined
    ...
    # get a list of all the internal Loops in the Structure object
    internal_loops = structure.internalLoops()

    return (rna_name, rna_id, rna_type, internal_loops)


def process_single_structure(structure_file, rna_identifiers, output_file):
    """
    Function: process_single_structure(structure_file, rna_identifiers, output_file)
    Description: Function to process a single .st file and write all bulge data to a file
    Parameters:
        (structure_file) -- str -- .st filename
        (rna_identifiers) -- dict -- dictionary of RNA Identifier data
    ...
    """
    try:
        structureObject = ST.Structure(
            structure_file)  # create Structure object
    except Exception as e:
        print(
            f'An error occurred when creating a Structure object for {structure_file}: {e}.')
        return

    rna_name, rna_id, rna_type, internalLoops = get_data_from_structure(
        structureObject, rna_identifiers)  # get data from Structure Object

    with open(output_file, 'a') as f:
        for internal_loop in internalLoops:
            internalLoopData = get_data_from_internal_loop(
                internal_loop, structureObject)
            if internalLoopData:  # make sure all energy data is present
                ...
                dataString = f'{rna_name}\t{rna_id}\t{rna_type}\t{internalLoopLabel}\t{internalLoopEnergy}\t{label5p}\t{energy5p}\t{label3p}\t{energy3p}\n'
                f.write(dataString)
...
def process_multiple_files(file_list, rna_identifiers, output_file):
    ...
    Description: Function to process a list of .st file and write all bulge data
                 to a file
    Parameters:

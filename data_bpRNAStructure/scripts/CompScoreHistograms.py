...
usage = 'python scripts/CompScoreHistograms.py <hairpin_scores.txt> <bulge_scores.txt> <internalloop_scores.txt> <1m90_IDs.txt>'
...
def read_data(filename,IDs1m90):
    data = []
    control_stems = []
    control_loops = []
    ...
    control = np.array(control_stems)+np.array(control_loops)
...
def make_histogram(Hdata,Bdata,Idata,d1m90,control):
    ...
    plt.legend()
    out = 'figures/compensation_scores'+str1m90+'.pdf'
...
def make_individial_histograms(Hdata,Hcontrol,Bdata,Bcontrol,Idata,Icontrol,Density):
    ...
    out = 'figures/compensation_scores_bytype_90.pdf'
    if Density:
...
def make_bars(Hdata,Hcontrol,Bdata,Bcontrol,Idata,Icontrol,d1m90):
    ...
    plt.legend()
    out = 'figures/compensation_scores'+str1m90+'_controlfc.pdf'
    print(out)
    plt.savefig(out)
    plt.clf()
    ...
    plt.xlabel(r'Net $\Delta$G (kcal/mol)',fontsize = 16)
    plt.xticks(list(range(len(bins)))[::2],list(bins)[::2])
    out = 'figures/compensation_scores'+str1m90+'_controlfc_inset.pdf'
    print(out)
    plt.savefig(out)
    plt.clf()
...
def make_violins(Hdata,Hcontrol,Bdata,Bcontrol,Idata,Icontrol,d1m90):
    ...
    for pc in violins['bodies']:
        ...
        del colors[0]
    out = 'figures/compensation_scores_violins'+str1m90+'.pdf'
    print(out)
    plt.ylim(-30,20)
    plt.savefig(out)
    plt.clf()
...
Hdata,Hcontrol = read_data(Hfile,d1m90)
Bdata,Bcontrol = read_data(Bfile,d1m90)
Idata, Icontrol = read_data(Ifile,d1m90)

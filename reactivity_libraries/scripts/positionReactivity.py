...
#function netEdict
#netEfile: log file from scripts/ReactivityAssignedDSCIheatmap.py containing 
...
usage = "usage: python "+sys.argv[0]+" <summary.json> <DMS log file> <path to RNA.ste>"
if len(sys.argv) != 4:
    print(usage)
    sys.exit()
...
loopChar = RNAstefile.strip('/').split('/')[0]
...
for i in range(len(bins)):
    ...
    up,down,stem = np.log(np.array(up)),np.log(np.array(down)),np.log(np.array(stemAvgs[i]))
...
plt.ylabel('log average stem position reactivity',fontsize = 16)
if loopChar[0] in 'Hh':loop = 'Hairpin'
if loopChar[0] in 'Bb':loop = 'Bulge'
if loopChar[0] in 'Ii':loop = 'Internalloop'

outfile = "figures/"+loop+"_perpositionStemReacLog.pdf"
print(outfile)
plt.savefig(outfile)

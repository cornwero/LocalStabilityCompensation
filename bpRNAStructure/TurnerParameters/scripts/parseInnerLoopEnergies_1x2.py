'''
Filename: parseInnerLoopEnergies_1x2.py
...
Description: Function parse the energy parameters for 1x2 internal loops from the turner parameter text file
...
def parseInnerLoopEnergies(filename):
    ...
    # values will be stored in dictioanry as (5' closing pair, 3' closing pair, x, y, z)
    ...
    for b in range(len(blocks)):
        ...
        values = blocks[b][10:14] #rows with parameter value
        ...
        for i in range(len(values)): #iterate through each line of values(4 lines)
            ...
            for j in range(len(lineContents)): #iterate through the list of values
                ...
                energyDict[labels[counter], labels[counter+1], bases[i], bases[j%4], bases[b%4]] = float(lineContents[j])
...
'''
def parseArgs():
    parser = argparse.ArgumentParser(description="Turner Parameters Parser for 1x2 Inner Loop Energies.")
